"""
backend/pipeline/overpass_client.py
─────────────────────────────────────
Robust Overpass API client.
- Exponential backoff on 429 / 504 / 503
- Tries 3 mirror URLs before giving up
- Returns raw elements list or []
"""
...
def query(ql: str, timeout_s: int = 60, retries: int = 3) -> list[dict]:
    ...
    Tries each mirror up to `retries` times with backoff.
    """
    ...
    log.error("  All Overpass mirrors failed — returning empty")
    return []
