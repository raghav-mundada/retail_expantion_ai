...
// Browser Geolocation wrapper — imperative locate() + UI state. Kept local
// to MapPicker because it's the only surface that needs GPS today.
...
export function MapPicker({ onAnalyze }: Props) {
  ...
  const { locate, busy: locating, error: geoError, clearError } = useGeolocation();
  ...
  return (
    <div className="relative h-[calc(100vh-4rem)] w-full overflow-hidden">
      ...
      {/* Pick panel — right side, header at top, form collapsible below */}
      <motion.div
      ...
      {/* Bottom-left — GPS action + context strip */}
